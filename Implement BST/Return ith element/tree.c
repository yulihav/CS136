...
Runtime is O(height t) because due to the structure of BSTs,
we do not need to visit each node. We recursively check how many
elements are in each subtree, and determine which path we need
to take that way (the size of each subtree tells us whether
we need to take the right or left path)

Hence, the worst case runtime will be the length of the longest 
path, which is the height of the tree by definition.
...
int kth(struct node * n, int k){
  //printf("n->left->size is %d\n",n->left->size);
  if(n == NULL) {
    return 0;
  }  
  if(n->left == NULL) {
  ...
  //if they key is the kth node
  if((n->left != NULL)&&(n->left->size == k)) {
    return n->key; 
  }
  // If there are more than k nodes in left sub tree, the smallest
  //must be in the left side
  else if((n->left != NULL)&&(n->left->size > k)) {
    return kth(n->left, k); 
  }
  // else, the kth smallest cant be in the left side 
  else {
    if(n->left != NULL) {
      return kth(n->right,k-(n->left->size)-1);
    }
    return n->right->key;
  }
}

 A10p3 -- return the ith element of t, where i=0 returns
the smallest element, i=1 returns the next smallest, and so on.
Requires that i < size(t).  O(height(t)) time. 
int ith(int i, struct inttree *t) {
  if (t == NULL) {
    return INT_MIN;
  }
  else {
...
// 3) ---------------------------------------------------
//Return the ith element of t, where i = 0 returns the smallest
//elements, i = 1 returns the next smallest, and so on
//Requires: i < size(t)

int kth(int i, struct node *n) {
  //if (n==NULL) return INT_MIN;
  
  int lsize;
  if (n->left == NULL) lsize = 0;
  else lsize = n->left->size;
  
	if (lsize == i) return n->key;

	else if (lsize > i) return kth(i, n->left);
	
	else return kth(i - lsize - 1, n->right);
}


int ith(int i, struct inttree *t) {
  if (t->top == NULL) return INT_MIN;
  else return kth(i, t->top);
}
